...
import { getSuspenseConfigValueQueryOptions } from '@/hooks/use-config-value/get-config-value-query-options';
import { getSuspenseDomainDescriptionQueryOptions } from '@/views/shared/hooks/use-domain-description/get-domain-description-query-options';
...
export default function DomainPageMetadata(props: DomainPageTabContentProps) {
  const [
    ...
    queries: [
      getSuspenseDomainDescriptionQueryOptions({
        domain: props.domain,
        cluster: props.cluster,
      }),
      getSuspenseConfigValueQueryOptions({
